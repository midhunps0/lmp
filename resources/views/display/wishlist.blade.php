...
<section id="products" class="w-full p-24 mb-20 ">
    ...
    <div class="product grid lg:grid-cols-4 gap-8 md:grid-cols-2  sm:grid-cols-2">
        ...
        <div id="product" class="relative transition-transform duration-300 ease-in-out">
            ...
            <form action="{{ route('removeWishlist' ,['product' => $wishlist->productvariant->id ])}}" method="post">  
                 @csrf
                <input type="hidden" name="product_variant_id" value="{{ $wishlist->productvariant->id }}">
            ...
            <div class="flex group">
                <div class="w-full relative transition-opacity duration-500 ease-in-out h-96">
                        ...
                        <img alt="{{asset('images/product_01.jpg')}}" src="{{  $wishlist->productvariant->getSingleMediaUrl('image') }}" class="w-full group-hover:hidden">
                        <img alt="{{asset('images/product_01.jpg')}}" src="{{  $wishlist->productvariant->getSingleMediaUrl('imageSecond') }}" class="w-full hidden group-hover:flex">
                    </a>
                </div>
            </div>
            <div class="product_details flex bg-white flex-col hover:shadow-md">
                ...
                <h4 class="p-2 text-lg">{{$wishlist->productvariant->name}}</h4>
                <p class="p-2 text-base">{{$wishlist->productvariant->price}}</p>
            </div>
